...
function detectMeeting(): boolean {
  ...
  if (platform === 'zoom') {
    ...
    // Zoom often leaves old modal nodes in the DOM after a leave/rejoin cycle.
    // Only a visible ended/left dialog should block URL-based meeting detection.
...
function isLikelyMeetingUrl(url: string = window.location.href): boolean {
...
function shouldTrustLikelyMeetingUrl(): boolean {
  ...
  switch (message.type) {
    ...
    case 'SHOW_PROMPT':
      ...
      if (
        ...
        (state.meetingSessionId || detectMeeting() || shouldTrustLikelyMeetingUrl())
      ) {
        state.meetingDetected = true;
